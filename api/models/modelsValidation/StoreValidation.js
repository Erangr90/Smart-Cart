...
const isValidStore = (obj) => {
    ...
    // if (!businessNumberRegex.test(obj.businessNumber)) { errors.push(`${obj.businessNumber} - business Number is not valid `); }
    if (!cityNameEnRegex.test(obj.address.city) && !cityNameHeRegex.test(obj.address.city)) { errors.push(`${obj.address.city} - City is not valid `); }
    if (!streetNameEnRegex.test(obj.address.street) && !streetNameHeRegex.test(obj.address.street)) { errors.push(`${obj.address.street} - Street is not valid `); }
    if (obj.address.postalCode && !postalCodeRegex.test(obj.address.postalCode)) { errors.push(`${obj.address.postalCode} - Postal Code is not valid `); }
    ...
    // if (!branchNumberRegex.test(obj.branchNumber)) { errors.push(`${obj.branchNumber} - Branch number is not valid `); }
