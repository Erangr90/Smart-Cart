...
const CreatePrice = () => {
  ...
  const selectStoreHandler = (e) => {
    ...
    setStoreId(obj._id);
    setStoreQuery(
      `${obj.name} - ${obj.businessNumber} ${obj.chain ? " - " + obj.chain.name : ""
      }`
    );
  };
  ...
  const selectChainHandler = (e) => {
    ...
    setChainId(obj._id);
    setChainQuery(
      `${obj.name} - ${obj.businessNumber} ${obj.stores ? "- " + obj.stores.length + "סניפים" : ""
      }`
    );
  };
            ...
            <Form.Group className="my-2">
              {stores && stores.stores.length > 0 && storeQuery.length > 1 ? (
                ...
                >
                  ...
                  {stores.stores.map((x) => (
                    <option value={JSON.stringify(x)} key={x._id}>
                      {`${x.name} - ${x.businessNumber} ${x.chain ? " - " + x.chain.name : ""
                        }`}
                    </option>
                  ))}
                </Form.Select>
            ...
            <Form.Group className="my-2">
              {chains && chains.chains.length > 0 && chainQuery.length > 1 ? (
                ...
                >
                  ...
                  {chains.chains.map((x) => (
                    <option value={JSON.stringify(x)} key={x._id}>
                      {`${x.name} - ${x.businessNumber} ${x.stores ? "- " + x.stores.length + "סניפים" : ""
                        }`}
                    </option>
                  ))}
                </Form.Select>
